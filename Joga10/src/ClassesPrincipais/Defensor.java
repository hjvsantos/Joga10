package ClassesPrincipais;

import java.util.Date;

public class Defensor extends Jogador {

	public Defensor(String nome, String posicao, double registro,
			String cidade, String estado, String cPF, double rG,
			Date dataNascimento, double telefone, String email, double passe,
			float altura, float peso, String timeAtual) {
		super(nome, posicao, registro, cidade, estado, cPF, rG, dataNascimento,
				telefone, email, passe, altura, peso, timeAtual);
	}

}
